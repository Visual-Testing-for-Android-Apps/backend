...
def extract_features(videoArray, net_name, ck_path, save_pca, logits_name, image_paths, save_dir, test_label = os.path.join(constants.DATA_PATH, "test_data", "images", "label.txt"), batch_size = 1, image_dims = (224, 224, 3), frames = 8, max_num_images = 225, _num_classes = 50):
  global num_classes
  ...
  features = []
  image_data = videoArray#_concat_image(constants.UNQUALIFIED_DATA_PATH, gif_files[i])
